...
function build_with_flags() {
  ...
  log "🔧 [$stage_name] Configuring build..."
  rm -rf "$build_dir"
...
function install_final_build() {
  log "🧹 Installing final build to: $PREFIX"
  sudo rm -rf "$PREFIX"
...
function build_spirv_llvm_translator() {
  log "📦 Building SPIRV-LLVM-Translator with PGO and optimization..."
  rm -rf "$ROOT"
  mkdir -p "$ROOT"
  cd "$ROOT"
  ...
  [[ -d "spirv-llvm-translator" ]] || fail "Repository not found after clone"
  ...
  success "llvm-spirv successfully built and installed to: $PREFIX/bin/llvm-spirv"
}
